...
public class Link { // Link类给外部调用
	...
	public class Node { // Node 类只给Link类调用，用于存储数据
		...
		public void eachNode() {
			Link.this.resultArr[Link.this.foot++] = this.data;
			if (this.next != null) {
				this.next.eachNode();
			}
		}
		...
		// 遍历节点(还是测试方法，方便理解的)
		public void eachNode(Object[] arr, Node root, Integer foot) {
			arr[foot++] = root.data;
			if (root.next != null) {
		...
		// 根据索引查找数据
		public Object searchObj(int index) {
			if (Link.this.foot++ == index) {
				return this.data;
			} else {
				return this.next.searchObj(index);
			}
		}
		 ...
		 */
		public void delete(Object object, Node node) {
		...
		// 判断里面是否有指定的数据
		public boolean searchStr(Object search) {
		...
		// 对指定索引的数据进行修改
		public void editVal(int index, Object val) {
			if (Link.this.foot++ == index) {
				this.data = val;
			} else {
				if (this.next != null) {
	...
	// 添加元素
	public boolean addEle(Object object) {
		if (object == null) {
			return false;
		}
	...
	public Object get2(int index) {
		...
		this.foot = 0;
		return this.root.searchObj(index);
	}
